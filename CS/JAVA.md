...
### 추상클래스
 ...
 추상 메소드가 하나 이상 포함되거나 `abstract`로 정의된 경우

- 일반 벼수들과 일반 메소드들도 쓸수 있고 아직 구현하지 않아도 될 메소드는 그냥 내버려둘 수 있어 상황에 따라 편리하다.

### 인터페이스

모든 메소드가 추상 메소드인 경우 (java8 이후에서는 default 키워드를 이용해서 일반 메소드의 구현도 가능하긴 함.)

- 디자인을 구성하는 요소들이 자주 바뀔때 쓰면 유용하다.

### 인터페이스와 추상 클래스의 차이

- 추상 클래스는 상속받아 기능을 이용하고, 확장시키는데 사용하지만 인터페이스의 경우 메소드의 구현을 강제하기 위해서 사용한다.
- 자바는 다중 상속을 지원하지 않기때문에 
